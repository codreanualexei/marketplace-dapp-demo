    ...
    try {
      ...
      if (provider) {
        ...
        if (code === "0x") {
          ...
          log(
            `✅ Contract exists at ${marketplaceAddr} (${code.length} bytes)`,
          );
        }
      }
      ...
      try {
        ...
        if (Number(lastId) > 0) {
          ...
          log(
            `✅ Listing 1: Seller=${listing.seller}, TokenId=${listing.tokenId}, Price=${ethers.formatEther(listing.price)} MATIC, Active=${listing.active}`,
          );
        } else {
      ...
      if (provider) {
        ...
        if (code === "0x") {
          ...
          log(
            `⚠️ Your NFT contract is NOT on the network you're connected to!`,
          );
          return;
        } else {
      ...
      try {
        ...
        if (tokenData) {
          log(
            `✅ Token 1 data: Creator=${tokenData.creator}, URI=${tokenData.uri}`,
          );
        }
      } catch (error: any) {
        if (
    ...
    <div className="debug">
      <div className="debug-container">
        ...
        <div className="debug-card">
          ...
          {!account && (
            <div className="warning">⚠️ Connect your wallet first</div>
          )}
